...
- **FINAL step-5 provider decision (supersedes the qwen/Claude attempts above)** — neither local qwen nor Claude-via-Bedrock-shim reliably satisfied okc-core's strict synthesis/critic contracts (exact identifier echoing; `metadata_` id rejected in a `BlockId` field). Since okc-core's own tests compile against a deterministic fixture provider, step-5 generation was pivoted to `demo/scripts/curation_provider.py` — a small input-aware **deterministic** provider that echoes identifiers exactly, groups near-duplicate note pairs, and preserves contradictions. Result: fully local/offline (no Ollama/Bedrock/API keys), reproducible, and `run-demo.sh --fresh` completes end-to-end publishing 2 revisions. Steps 1–4 remain real; step-5 merge content is deterministic-synthetic (labeled in the runbook). See [build-and-test summary](../../construction/build-and-test/demo-full-flow-build-and-test-summary.md).
