...
okc-web는 Obsidian Vault 통합 웹 플랫폼(GREENFIELD, 해커톤 PoC, 수상 목표)이며, FastAPI(Python)가 okc Python 바인딩(`okc-compiler`)을 직접 import(maturin path install)하고 React + Vite SPA+Tailwind+shadcn/Tremor 프런트엔드로 서빙한다(rebased to FastAPI+React SPA per ADR-0025). 본 계획은 3개 렌즈(demo-velocity / judging-criteria / engineering-correctness)를 조정하여 도출한 단일 권위 실행 계획이다.
...
- **Structural 변경**: Yes — 단일 장기 실행 엔진 프로세스 + okc 바인딩 어댑터 경계(ADR-0002), 프로세스-글로벌 scheduler/PROJECT_RESERVATIONS를 감싸는 in-process 단일-writer 직렬화 큐가 신규 구조.
...
- [EXECUTE] **NFR Requirements** — depth: minimal
  - Rationale: 2/3 렌즈 EXECUTE. 스택 확정(FastAPI+okc 바인딩 path install)이고 NFR은 requirements.md에 이미 열거(NFR-CONC-1/SEC-1/PERF-1/PORT-1/AVAIL-1)돼 신규 elicitation 불필요. 그러나 직렬화 NFR은 미할당하기엔 너무 load-bearing하고, C6 채점(구조적 로깅/관측성, 설정 분리, 시크릿 위생)이 있으므로 SKIP 대신 minimal로 실행: 기존 NFR을 소유 유닛에 바인딩 + 로깅/config-separation/secret-hygiene를 명문화.
- [EXECUTE] **NFR Design** — depth: standard
  - Rationale: 2/3 렌즈 EXECUTE. 최고가치 정합성 패턴이 구체화되는 지점: in-process 단일-writer 큐, PROJECT_BUSY retry/queue, 해시 바인딩 staleness 탐지+캐스케이드 무효화, OkcError code/category 분기(메시지 파싱 금지), schema-version 거부, publish barrier 전 취소, 해시 시크릿 저장(argon2id/`argon2-cffi`), 스트리밍 업로드 검증 파이프라인. Resiliency Baseline opt-out이므로 comprehensive는 과함 → standard.
...
- [EXECUTE] **Code Generation** — depth: comprehensive — ALWAYS, per-unit
  - Rationale: 3개 렌즈 EXECUTE. C4(actually-works, 최고 가중, 하드 게이트)가 여기서 승부. FastAPI(uvicorn)+React(Vite) 엔트리포인트, 커밋된 lockfile(`uv.lock` + `package-lock.json`), 실제 okc 바인딩 배선(add_source/approve_taxonomy/approve_cluster/regenerate_cluster/compile/verify/explain — 모킹 금지), RBAC-before-core 게이트(FastAPI `Depends`), APPROVAL_REQUIRED compile 거부, no-clobber 서빙, 글로벌 에러 핸들러, 22 Must 데모 스파인에 stub/TODO 금지. Should-only(E2-S6/E3-S5/E4-S5/E5-S5)는 경량 stub/deferred. Part 1 체크박스 계획으로 C1 추적성 유지.
...
- **U0 — Platform & okc-bindings Adapter** (공유 기반, 최우선): 단일 격리 okc 바인딩 어댑터(OkcClient/Project/Job 래핑, typed-DTO interop schema v2 경계, schema-version 가드), 단일 장기 엔진 프로세스 + in-process 단일-writer 직렬화/PROJECT_BUSY 큐, okc-core commit-pin 소스 빌드(maturin), 횡단 관심사: 단일 authz 가드(FastAPI `Depends`), 단일 OkcError→HTTP 매핑, 재사용 업로드 입력검증 모듈, curator-decision append-only 감사 저장소, 구조적 로깅/correlation-id. (Cross-cutting; E3/E4 기반, C-1/C-4/C-5/FR-INT-8/NFR-CONC-1/PORT-1)
...
| Code Generation (comp.) | 모듈 story-ID 태깅 | N/A | 실 okc 바인딩 배선 검증 | 실행 코드, no-stub 스파인 | 상태 처리 구현 | RBAC-before-core, 에러 핸들러 |
...
## Success Criteria
- **Primary Goal**: 22 Must 스토리의 데모 스파인이 실제 okc 바인딩 데이터로 end-to-end 동작하고, E4(충돌/크리틱, winner-select 없음·Major/Critical 비-waivable) 및 E5(provenance/verify) focal 화면이 실 파이프라인 산출물을 렌더 → C3 구조적 차별화가 코드/설계로 검증됨.
- **Key Deliverables**: (1) 실행되는 FastAPI(uvicorn)+React(Vite) 앱 + 커밋된 lockfile + 엔트리포인트; (2) okc-core commit-pin + both-stack CI 그린; (3) RBAC-before-core 게이트 + 글로벌 OkcError 핸들러; (4) in-process 단일-writer/PROJECT_BUSY 큐; (5) 루트 README(features=스크린샷, Problem Statement 축자, okc-mcp contract-only) + AI-DLC PROCESS narrative.
