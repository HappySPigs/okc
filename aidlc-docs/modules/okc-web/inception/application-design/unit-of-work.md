...
**Depth**: Standard
**System**: okc-web = okc-core 엔진 위에 얹은 Python(FastAPI) + React(Vite SPA) 컨트롤-플레인·서빙 계층. `okc` Python 바인딩(`okc-compiler` 0.3.0, maturin path install)을 import(ADR-0002), 단일 장기 프로세스(uvicorn one worker)·단일 SQLite 상태 파일·단일-writer 엔진 큐.
...
### U0 — Foundation (Platform & okc Bindings Adapter)

- **모듈**: `app/adapter`(+`adapter.queue`) · `app/shared`(`authz`/`error`/`jobs`/`audit`/`state`)
- **Epic**: cross-cutting — 특정 epic 소유 아님. E1–E5 전 유닛의 공유 기반이며, 특히 E3 오케스트레이션·E4 리뷰의 토대.
- **핵심 책임**:
  - 유일한 `okc` 바인딩 import 지점(ADR-0002): `adapter.OkcEngine` Protocol + 단 하나의 `OkcEngineImpl`. okc-web 소유 typed-DTO(`*View`/`*Cmd`/`*Spec`)와 `from_native(payload: dict)`(`okc.*` dict → Pydantic) 변환으로 downstream을 바인딩 churn에서 격리.
  - `SchemaGuard`: `okc.INTEROP_SCHEMA_VERSION == 2`를 client init 및 매 DTO decode에서 단일 지점 검증; 불일치 시 `EngineError(code="SchemaUnsupported")`(silent mis-decode 금지).
  - 오류 경계: 모든 `okc.OkcError`를 `adapter` 내부에서 okc-web 소유 `EngineError(code, category, retryable, retry_after_ms, message)`로 변환 — 바인딩 타입이 U1–U6로 넘어가지 않음.
  - `adapter.queue` 단일-worker 엔진 스레드: 단일-소유 **blocking** 워커(`ThreadPoolExecutor(max_workers=1)`, 유일 `OkcClient` 소유) + asyncio 브리지(`loop.run_in_executor`). reserving/mutating op을 정확히 하나씩 직렬화(NFR-CONC-1), okc 프로세스-글로벌 `PROJECT_RESERVATIONS` 위의 단일 writer. 잔여 `ProjectBusy`는 **code 기준**으로 409(+`Retry-After`).
  - read-path `OkcClient`(두 번째 인스턴스 + 소형 `ThreadPoolExecutor`): 비-reserving read(taxonomy/clusters/manifest/verify/explain)는 큐 우회(verifier #8, HOL-blocking 회피).
  - `shared.authz`: RBAC-before-core 가드·`Depends` 의존성·canonical `AuthContext(principal, role)`; `Role.{Admin, Contributor}`; `Principal.{Admin, UploadToken}`. **불변식(테스트 가능)**: 403/401은 `OkcEngine` 메서드가 0회 실행됐음을 보장(C-1).
  - `shared.error`: 단일 table-driven(dict) `EngineError` code/category → HTTP 매핑(`ProjectInvalid`/`ArtifactSchemaUnsupported`/`PathUnsupported`/`OutputDurabilityUncertain` + 안전 기본 500 포함); **message 문자열 파싱 금지**.
  - `shared.jobs`: `JobStore` + `job_events`(Q7 폴링 스냅샷의 단일 소스).
  - `shared.audit`: append-only `curator_decisions`(record-then-act, `HashBindings(proposal_hash, critic_hash, taxonomy_hash)`); discriminated-union 제약으로 **winner-select row 자체가 표현 불가**(C3).
  - `shared.state`: 단일 SQLite WAL 파일(`StateDb` = SQLAlchemy 2.0 `Engine` over sqlite3·마이그레이션·레지스트리).
...
- **okc-core 접점**: 전체 `okc` 바인딩 표면의 **유일 import 지점** — `okc.OkcClient`/`okc.Project`/`okc.Job`. `okc-compiler` 0.3.0 maturin path install, CI 소스 빌드(NFR-PORT-1).
...
- **핵심 책임**:
  ...
  - session resolution: `AuthContext` `Depends` 의존성 → `SessionStore.resolve`(TTL/idle) → `touch`(sliding). 산출 `Principal.Admin(account_id, session_id_hash, curator_label)`을 U0 RBAC 가드에 공급.
...
- **핵심 책임**:
  ...
  - `UploadIngestService`(capability-authenticated, U2 유일 mutating flow) — 순서 보장 fail-fast 파이프라인(add_source 이전 모든 단계는 거부 시 core-untouched 보장, C-1):
    1. `UploadContext` `Depends` 의존성(TOKEN_INVALID/EXPIRED/REVOKED → 401/403 before core).
...
- **핵심 책임**:
  - `CuratorDecision`(정확히 3-멤버 discriminated union, 리뷰 상태 변경의 유일 경로): `ApproveTaxonomy` / `ApproveCluster` / `RegenerateCluster` — **`SelectWinner`/`ResolveContradiction` 변형 없음**(code-verifiable C3 차별자).
  ...
  - `DecisionGate.assert_approvable`: Major/Critical 도메인 검사를 okc-web에서 수행 → `approve_cluster` 엔진 호출 이전에 `422 APPROVAL_REQUIRED`. **code-verified 필요성**: core의 blocking-approval 거부는 바인딩의 message 매핑을 거쳐 generic `ProjectInvalid`(not `ApprovalRequired`)로 매핑되므로, 결정론적 422는 okc-web 게이트에서 발생해야 함.
...
- **경계 (소유하지 않음)**:
  ...
  - taxonomy/clusters read는 U0 read-path client 경유(큐 우회) — U4는 소비.
- **okc-core 접점**: `approve_taxonomy`, `approve_cluster`, `regenerate_cluster`(U0 큐 경유); `taxonomy`/`clusters` read(U0 read-path client, 큐 우회).
...
- **모듈**: `web`(React + Vite SPA, React Router v6)
...
- **핵심 책임**:
  ...
  - 단일 client 컴포넌트 트리(shell/static + review/upload/polling); 데이터는 TanStack Query(server-component 분리 불필요).
...
- **okc-core 접점**: 없음(FastAPI HTTP REST + JSON only).
...
**결정(unit-of-work-plan Q6=A)**: 모노레포 준비형 단일 Python 패키지 백엔드 + React(Vite) 프론트엔드. **유닛 ↔ 백엔드 모듈 1:1**, 계층형(handler/service/repo) 대신 유닛-경계 정렬로 C1 추적성·C6 모듈화를 확보.

### 2.1 디렉터리 구조(현재 okc-web 루트)

```text
okc-web/
├── backend/                          # 단일 Python 패키지 (FastAPI)
│   ├── pyproject.toml                # okc-compiler(okc 바인딩) path dep (PEP 621) · lockfile 커밋
│   ├── uv.lock                       # 재현 빌드 (NFR-PORT-1)
│   └── app/
│       ├── main.py                   # FastAPI app factory: 라우터 조립 + 엔진 워커 spawn + StateDb init
│       ├── config.py                 # 설정 (경로·포트·env-var 이름)
│       ├── adapter/                  # U0 — 유일한 okc 바인딩 import 지점
│       │   ├── engine.py             #   OkcEngine Protocol + OkcEngineImpl (유일 okc.* import)
│       │   ├── dto.py                #   *View / *Cmd / *Spec + from_native(payload: dict)
│       │   ├── schema_guard.py       #   SchemaGuard (okc.INTEROP_SCHEMA_VERSION == 2)
│       │   └── queue.py              #   adapter.queue — 단일-worker 엔진 스레드 (ThreadPoolExecutor(1) + asyncio 브리지)
│       ├── shared/                   # U0 — 횡단 관심사
│       │   ├── authz.py              #   RBAC-before-core Depends · AuthContext · Role · Principal
│       │   ├── error.py              #   EngineError code/category → HTTP (단일 table-driven dict)
│       │   ├── jobs.py               #   JobStore + job_events (폴링 스냅샷)
│       │   ├── audit.py              #   append-only curator_decisions (record-then-act)
│       │   └── state.py              #   SQLAlchemy 2.0 StateDb (sqlite3 WAL) · 마이그레이션 · 레지스트리
...
├── frontend/                         # U6 — React + Vite SPA (shadcn/Tremor, UI 선착수)
│   ├── index.html · vite.config.ts   #   Vite 엔트리 + dev 프록시(/api,/u → uvicorn)
│   ├── package.json · package-lock.json  #   프론트 lockfile 커밋
│   └── src/                          #   React Router v6 라우트(E1–E5, client components)
...
|---|---|---|---|---|---|
| **U0** | `app/adapter`(+`queue`) · `app/shared`(`authz`/`error`/`jobs`/`audit`/`state`) | cross | 유일 바인딩 import 지점 · 단일-writer 큐 · RBAC 가드 · OkcError→HTTP · JobStore · 감사 · SQLite | okc.OkcClient/Project/Job (유일 import) | — |
| **U1** | `app/auth` | E1 | 계정·역할·세션·비밀번호 해시 | 없음 (curator_id 라벨만 U3 경유) | standard |
| **U2** | `app/upload` | E2 | 토큰 발급/폐기 · 토큰 인증 업로드 · 검증 · landing · ≤10 cap | `add_source` (큐 경유) | standard |
| **U3** | `app/orchestration` | E3 | lifecycle·freeze·checkpoint 루프·compile·staleness·`sources` 레지스트리 | `create_project`/`open_project`·`status`·`preflight`·`integrate`·`compile` (큐 경유) | comprehensive |
| **U4** | `app/review` | E4 | taxonomy/cluster 결정 표면 · `DecisionGate`(Major/Critical→422) · regenerate | `approve_taxonomy`·`approve_cluster`·`regenerate_cluster` (큐) · `taxonomy`/`clusters` (read-path) | comprehensive |
| **U5** | `app/serving` | E5 | read-only compiled-vault API · verify/explain provenance · publish · mcp 계약 | `verify_artifact`·`explain_artifact`·`manifest` (비-mutating, 큐 우회) | skip |
| **U6** | `web`(React+Vite) | cross | API 소비층(`apiClient`·`okcErrorMap`·`queryKeys`·`useJobPolling`) · 화면 배선 | 없음 (HTTP only) | skip |

### 2.3 모노레포 준비 레이아웃 (module-integration-guide §4 정합)

현재 `okc-web/`는 이미 `backend/` + `frontend/` + `aidlc-docs/`를 프로젝트 루트 하위에 중첩한다. 따라서 향후 모노레포 편입은 **`okc-web/` 폴더를 모노레포 루트 하위로 그대로 이동**하고 `okc` 바인딩(`okc-compiler`) path 경로만 재배선하면 되며, 내부 구조 재편은 불필요하다.

```text
<monorepo-root>/
├── okc-core/                 # Rust 엔진 — 핀된 의존성 (git submodule 또는 vendored, 상류 churn 격리)
├── okc-web/                  # 이 프로젝트가 그대로 이동
│   ├── backend/              # FastAPI 백엔드 (okc-compiler path dep → ../../okc-core/bindings/python)
│   │   └── app/{adapter,shared,auth,upload,orchestration,review,serving}/
...
**핵심 원칙**: (1) okc-core는 핀된 외부 의존성으로 유지(okc-web은 소비자, ADR-0002 무수정). (2) `okc` 바인딩 import는 `adapter` 단일 지점 — 나머지 모듈은 okc-web typed-DTO(interop schema v2)에만 의존. (3) provider 자격증명은 env-var **이름**으로만 참조(값 커밋 금지). (4) 로컬 절대경로 기반 landing/compile은 모노레포 이동 시 config로 외부화.
...
- **C6 (유지보수성 / 모듈화)** — 5-epic 1:1 모듈 분할 + U0 공유 기반 + U6 프레젠테이션으로 관심사 분리. 유닛 간 결합은 오직 (a) `OkcEngine` Protocol/타입 경계, (b) U0 `adapter.queue` single-writer 큐, (c) `shared.state` SQLite 레지스트리로만 이뤄지며 유닛 간 네트워크 호출이 없다(단일 프로세스, Q2/Q4=A). `okc` 바인딩 import는 `adapter` 단일 지점으로 격리(ADR-0002 import-경계 강제). 의존은 U0 기반 단방향으로 순환이 없으며(상세·Mermaid는 `unit-of-work-dependency.md`), `backend/app/{module}/` 1:1 디렉터리 + 모노레포 준비 레이아웃이 이 경계를 물리적으로 반영한다.
