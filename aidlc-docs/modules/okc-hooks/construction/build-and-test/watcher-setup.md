...
Fill in a copy of [`../../../examples/config.example.json`](../../../../../okc-hooks/examples/config.example.json) (the fields above; `vault_path`/`data_dir` must be absolute, `server_endpoint` must be `https`, `token` is the okc-web **upload token**), then run:
