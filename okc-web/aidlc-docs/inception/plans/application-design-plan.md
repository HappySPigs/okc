...
A) **단일 FastAPI 앱(Python 패키지, uvicorn 1 worker) + 유닛 경계별 모듈**(`adapter`, `auth`, `upload`, `orchestration`, `review`, `serving`, `shared`) — okc Python 바인딩(`okc-compiler`)은 path install. 해커톤 단순성 + 유닛↔모듈 1:1 추적성(C1/C6) — 권장 (rebased to FastAPI+React SPA per ADR-0025)

B) 멀티 패키지 워크스페이스(유닛별 서브패키지 + 얇은 진입점) — 경계 강제 강하나 해커톤엔 과함
...
## Question 2 — okc 바인딩 어댑터 인터페이스 스타일은? (ADR-0002 경계)
okc-web 나머지 코드가 okc 바인딩 타입에 직접 의존할지, Protocol/ABC 뒤로 숨길지.

A) **Protocol/ABC 경계**(`OkcEngine` Protocol) — 하나의 concrete impl이 okc 바인딩을 래핑, 나머지 모듈은 Protocol+okc-web 자체 typed-DTO(Pydantic)에만 의존. ADR-0002 어댑터 원칙 강제 + schema-version 가드 지점 단일화 + 테스트 mock 용이 — 권장

B) okc 바인딩 타입 직접 사용(어댑터 얇게) — 빠르나 경계 흐려짐·mock 어려움·schema drift 노출
...
A) **SQLite**(단일 파일, 임베디드, 트랜잭션) — 단일 프로세스 토폴로지에 정합, 동시성 안전(WAL), 스키마·마이그레이션 명확. SQLAlchemy 2.0 Core over sqlite3 — 권장
...
A) **서버측 세션**(HTTP-only·SameSite 쿠키에 opaque 세션 ID, 세션 레코드는 SQLite) — 즉시 폐기 가능·JWT 키관리 불필요·데모 단순, 비밀번호는 argon2id(`argon2-cffi`) 해시(NFR-SEC-1) — 권장
...
A) **단일 소유 in-process 워커**(엔진 핸들을 소유한 1개 워커 스레드 = `ThreadPoolExecutor(max_workers=1)` + 작업 큐; 모든 변경성 엔진 op이 이 큐를 통과, asyncio는 `run_in_executor`로 브리지) — 동시 요청은 큐잉, 코어 경합 시 `PROJECT_BUSY`→재시도 안내. 단일-writer 정확성 보장 — 권장
...
A) **명시적 `CuratorDecision` (Pydantic discriminated union)** {`ApproveCluster{omission_rationales, minor_waivers}`, `RegenerateCluster{feedback}`, `ApproveTaxonomy{edited_clusters, rationale}`} — append-only 감사 저장소에 기록 후 okc 바인딩 `approve_cluster`/`regenerate_cluster`/`approve_taxonomy`로 매핑. winner-select 변형 부재를 타입으로 강제(C3 차별자·ADR-0024) — 권장
...
프런트엔드(React + Vite SPA)와 okc-mcp가 소비할 HTTP 계약.
...
A) **React + Vite SPA(React Router v6)**(단일 클라이언트 트리; 셸/리뷰/업로드/폴링 모두 클라이언트 컴포넌트, 데이터는 TanStack Query), **TanStack Query**로 데이터·Job 폴링, shadcn/ui+Tremor+Lucide, FastAPI JSON API 소비 — 권장 (rebased to FastAPI+React SPA per ADR-0025)

B) 라우터 미사용 수동 라우팅 / SSR 프레임워크 재도입 — 선착수 IA(3-tier 라우트의 선언적 매핑)와 덜 정합
...
- [ ] ADR-0002 어댑터 경계: `OkcEngine` Protocol/ABC + typed-DTO(Pydantic) interop schema v2 가드 지점 명시
- [ ] RBAC-before-core 게이트 배치: 모든 변경성 op이 okc 바인딩 호출 **이전** 인가, 403=코어 미접근 보장
...
|---|---|---|---|
| `adapter` (OkcEngine Protocol+impl) | U0 | okc 바인딩 래핑, schema-version 가드, DTO 변환 | OkcClient/Project/Job |
...
| `web` (React + Vite SPA) | U6 | 화면 배선, 폴링, 상태 매트릭스 | (API 소비) |
