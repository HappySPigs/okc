...
**Depth**: Standard
**Date**: 2026-09-08
**System**: okc-web = 단일 FastAPI(Python) 백엔드 + React(Vite SPA) 프런트, `okc` Python 바인딩(`okc-compiler` 0.3.0) import로 okc-core 엔진 소비(ADR-0002). 단일 장기 프로세스(uvicorn one worker) · 단일 SQLite state · 단일-writer 엔진 큐.
...
| **U6** Frontend (`web` / React+Vite) | X | X | X | X | X | X | — |
...
### 외부 의존성 — okc 바인딩 (유닛 아님)
`okc` 바인딩(`okc-compiler` 0.3.0, → okc-core 엔진)은 유닛이 아니라 maturin path install된 외부 의존성이다. **오직 U0 `adapter`만이 이를 import한다**(ADR-0002 단일 import 지점). U1–U6는 U0가 소유한 typed-DTO(`*View`/`*Cmd`/`*Spec`) + `EngineError` 경계 뒤에서만 엔진에 도달하며, 어떤 `okc.*` 타입도 U0 경계를 넘지 않는다. 따라서 매트릭스 열에는 U0–U6만 두고 `okc` 바인딩은 U0의 외부 의존으로 별도 표기한다.

### 유닛별 의존 근거 (component-dependency.md 매트릭스 기준)
- **U0 → (없음)**: 기반 유닛. 유닛 내부에서 `adapter`가 `shared.jobs`/`shared.audit`를 참조하나 이는 U0 내부(intra-unit) 결합이며 다른 유닛 의존이 아니다. 외부로는 `okc` 바인딩만 import한다.
...
모든 유닛은 **하나의 FastAPI 앱(단일 Python 패키지) 안의 인-프로세스 모듈**이다(`unit-of-work-plan.md` Q2/Q4 = A). **유닛 간 네트워크 호출·프로세스 간 코디네이션은 없다** — 단일 장기 엔진 프로세스가 `okc` 바인딩의 프로세스-글로벌 `scheduler`/`PROJECT_RESERVATIONS`와 정합한다. 통신은 아래 5가지 패턴으로 한정된다.

1. **Protocol 경계 (인-프로세스)** — U1–U6는 U0 `adapter.OkcEngine` Protocol + okc-web DTO에만 의존한다. `okc` 바인딩은 `adapter`에서만 import되어 ADR-0002가 import 경계로 강제된다. 인-프로세스 함수/메서드 호출이므로 유닛 간 RPC/HTTP가 없다.
2. **직렬화 큐 (단일-worker 엔진 스레드) — 모든 변경성 엔진 op** — reserving/mutating op은 handler → `EngineHandle`(async 제출) → 단일 blocking 워커(`ThreadPoolExecutor(max_workers=1)`, 유일 `OkcClient` 소유자) → `okc` 바인딩 경로로 흐른다. 워커는 한 번에 정확히 하나의 명령만 처리하고 `okc.Job`을 terminal까지 몰아간 뒤 다음을 dequeue한다 → okc-web이 엔진의 **단일 writer**(NFR-CONC-1). 잔여 `ProjectBusy`(진짜 cross-process 경합)는 **code로 분기**하여 409 + `Retry-After`로 처리한다(바인딩 `retryable` 플래그 미신뢰).
...
| **큐 우회 (read-path OkcClient, 두 번째 인스턴스)** | non-reserving **read** | `taxonomy`, `clusters`, `manifest`, `verify`, `explain` | U4(taxonomy/clusters 읽기) · U5(manifest/verify/explain) |

**핵심 불변식**: (a) 유닛 간 네트워크 호출 없음(단일 Python 패키지 인-프로세스). (b) 모든 변경성 엔진 op은 U0 단일-writer 큐로 직렬화. (c) 유닛 간 공유 상태는 U0 `shared.state`(SQLite) 경유만. (d) U6는 HTTP API만 소비. (e) 비변경 `verify`/`explain`(및 taxonomy/clusters/manifest 읽기)은 큐를 우회해 다분 걸리는 integrate/compile 뒤에서 stall되지 않는다.
...
|---|---|---|---|
| 1 | **U0** Foundation | (기반) | **모든 유닛이 U0에 의존**하므로 최우선. 유일한 `okc` 바인딩 import 지점 + 단일-writer 큐 + RBAC 가드 + `OkcError→HTTP` 매핑 + `JobStore` + 감사 + SQLite를 선착지. 최고 리스크(`okc-compiler` maturin 소스 빌드/CI 그린)를 먼저 차단한다. 유닛 내부 순서는 `shared`(state/error/authz/jobs/audit) → `adapter`(+`queue`). |
...
간선 방향은 **의존하는 쪽 → 의존받는 쪽**(A → B = "A는 B에 의존")이며, 모든 간선이 기반 U0(및 그 아래 `okc` 바인딩 → okc-core)로 수렴한다. 역방향 간선이 없어 순환이 없다.

```mermaid
flowchart TD
    U6["U6 Frontend web (React+Vite) HTTP consumer"]
    U1["U1 Auth"]
    U2["U2 Upload"]
    U3["U3 Orchestration"]
    U4["U4 Review (wow)"]
    ...
    INTEROP["okc bindings (okc-compiler, maturin path install)"]
...
- **U0 → okc 바인딩 (import, sole link)**: U0 `adapter`만이 `okc` 바인딩(`okc-compiler`)을 import한다.
- **okc 바인딩 → okc-core**: 바인딩이 okc-core 엔진을 래핑한다.
...
검증 근거:
1. **U0 기반성**: §1 매트릭스의 U0 행은 전부 빈칸이다 — U0는 어떤 유닛에도 의존하지 않는다. 반대로 U1–U6의 U0 열은 전부 `X`다 — **모든 유닛이 U0에 의존**한다. U0의 유일한 바깥 의존은 유닛이 아닌 외부 `okc` 바인딩이다.
2. **하삼각 = 순환 없음**: §1 매트릭스는 하삼각(대각선 위 `X` 없음)이므로 어떤 유닛도 자기보다 나중에 빌드되는 유닛에 의존하지 않는다. §4 그래프의 모든 간선은 U0(및 `okc` 바인딩 → okc-core) 방향으로만 수렴하며 역방향 간선이 없다.
