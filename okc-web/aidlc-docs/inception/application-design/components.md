...
**Grounding**: okc-interop verified at `c:/Users/genie/workplace/okc-core/crates/okc-interop/src/lib.rs` (INTEROP_SCHEMA_VERSION=2). UI is frozen by `aidlc-docs/inception/application-design/ui-screens.md` + `design-system.md` — this document defines the API-wiring contract only and does **not** redesign UI.

> **Verifier corrections applied (see Application Design §"Applied Corrections" for the full list):** trait gains `preflight` + `status`; `create_project`/`open_project` split; `Role::{Admin,Contributor}` (no `Curator`/`UploadAgent`); `Principal::UploadToken(UploadContext)`; single `EngineError` at the adapter boundary (no `okc_interop` type past `adapter`); one project-scoped job-status route; non-reserving reads bypass the queue via a read-path client clone; unified verify/explain DTO names (`VerificationView`/`ProvenanceView`); explicit `sources` registry owner; `curator_decisions` carries all three hash bindings; extended HTTP mapping table.
...
|---|---|---|---|
| `adapter` (+ `adapter::queue`) | U0 | E3 support | sole `okc-interop` link; trait, DTOs, schema guard, engine actor/queue |
...
| `web` (Next.js) | U6 | cross | API-consumption layer (client, error map, query hooks, job polling) |

---

## U0 — `adapter` + cross-cutting `shared` (Foundation)

U0 is the SOLE point that links `okc-interop`. Every other unit depends only on the `OkcEngine` trait, okc-web-owned typed DTOs, `EngineError`/`ApiError`, the `JobStore` polling surface, and the authz guard — **never on `okc_interop::*`**.

### C0.1 `adapter::OkcEngine` (trait) — sole engine boundary
- **Responsibility**: the one abstract seam over okc-core. Declares async, okc-web-DTO-typed operations for the whole lifecycle. No caller outside `adapter` may name an `okc_interop` type (ADR-0002).
- **Interface**: `#[async_trait] pub trait OkcEngine: Send + Sync`. Read ops return a `*View` DTO; long mutations return `JobId`; fast mutations return `JobOutcome`. **All errors are the okc-web-owned `EngineError`** (never `okc_interop::OkcError`).
...
- **Responsibility**: the only struct holding an `okc_interop::OkcClient`. Translates trait calls into interop calls and converts interop result/error types into okc-web DTOs/`EngineError` at this boundary. Holds no engine mutex: the `EngineActor` owns the live handle; `OkcEngineImpl` holds a cloneable `EngineHandle` (for reserving/mutating ops) **plus a read fast-path `OkcClient` clone** used for non-reserving reads (queue-bypass, see Services §read-path policy).
- **Interface**: `impl OkcEngine for OkcEngineImpl`; `OkcEngineImpl::new(handle: EngineHandle, reader: OkcClient, guard: SchemaGuard) -> Self`.

### C0.3 `adapter::dto` — okc-web-owned typed DTOs (interop schema v2 mirror) + conversions
- **Responsibility**: define okc-web's own DTOs and `From<okc_interop::…>` conversions; the only translation layer that references interop result types. Downstream units decouple from interop churn behind this boundary.
...
- **Responsibility**: the ONE place asserting compatibility with `okc_interop::INTEROP_SCHEMA_VERSION` (==2). Checked at client init against `OkcClient::api_info().interop_schema_version` and again at each DTO decode boundary. Mismatch aborts startup or yields `EngineError{code: SchemaUnsupported}` rather than mis-decoding.
- **Interface**: `assert_client(&OkcClient) -> Result<(), EngineError>`; `check_payload(version: u32) -> Result<(), EngineError>`.

### C0.5 `adapter::queue::EngineActor` + `EngineHandle` — single-owner worker (NFR-CONC-1)
- **Responsibility**: a dedicated **blocking** worker that exclusively owns the process's single `OkcClient` and a cache of `Project` handles keyed by canonical path. Receives commands over a bounded tokio mpsc queue; processes them ONE AT A TIME; drives each interop `Job<T>` to terminal (pumping `Job::events()` into the `JobStore`, then reading the blocking `Job::result()` on its own thread). Serializes **only reserving/mutating** engine ops (all mutations plus the reserving `status()`); non-reserving reads take the read-path clone and never enqueue (avoids head-of-line blocking behind a multi-minute `integrate`/`compile`).
- **Interface**: `EngineHandle{tx}` (Clone, held by handlers); `EngineHandle::call(op) -> Result<EngineValue, EngineError>` (await terminal, fast ops); `EngineHandle::enqueue(op) -> Result<JobId, EngineError>` (id immediately, worker runs async, client polls); `EngineActor::spawn(client, jobs, audit) -> EngineHandle`.
- **Code-verified rationale**: `okc_interop::Job::result()` blocks on a Condvar (lib.rs wait) → the owner MUST be a dedicated blocking thread, not a plain tokio task (reconciles Q6 "one task owns the handle"). Concurrency is also enforced by a process-global `PROJECT_RESERVATIONS`; keeping exactly one `OkcClient` here makes the okc-web queue the primary serializer and `PROJECT_BUSY→409` the residual safety net.

### C0.6 `shared::jobs::JobStore` — job state + progress (FR-INT-8 / Q7)
- **Responsibility**: authoritative okc-web job state persisted in SQLite (`jobs` + `job_events`) plus an in-memory live snapshot. Updated by `EngineActor` from interop `ProgressEvent`s (coalesced upstream at 64). Read by the single project-scoped job-status endpoint that TanStack Query polls.
- **Interface**: `create(project, operation) -> JobId`; `record_progress(&JobId, &ProgressView)`; `record_terminal(&JobId, Result<(), EngineError>)`; `get(&JobId) -> Option<JobSnapshot>`. `JobSnapshot{id, project, operation, state: JobStatus, phase, completed, total, current_item, error: Option<ApiErrorBody>, updated_at}`; `JobStatus` mirrors interop `{Queued, Running, Cancelling, Publishing, Completed, Failed, Cancelled}`.

### C0.7 `shared::authz` — RBAC-before-core guard (C-1)
- **Responsibility**: tower/axum middleware + extractors that authenticate the caller and enforce role BEFORE the handler runs, so a denied request never reaches an engine call (a 403 guarantees okc-core was not touched). U0 owns the enforcement machinery + ordering guarantee; U1/U2 supply concrete validators via an injected `AuthProvider`.
- **Canonical types (corrected)**:
  - `enum Role { Admin, Contributor }` — no `Curator`, no `UploadAgent`. "Curator" is the capacity in which an admin acts, recorded as okc-core's unverified `curator_id` label (FR-AUTH-4). Upload is a *capability*, modeled as `Principal::UploadToken`, not a role.
  - `enum Principal { Admin{ account_id, session_id_hash, curator_label }, UploadToken(UploadContext) }`.
  - `struct AuthContext { principal: Principal, role: Role }` — the session-authenticated principal for admin routes (U0-owned canonical type; U1's `account_id`/`curator_label`/`session_id_hash` fold into `Principal::Admin`).
  - `struct UploadContext { token_id, project_id, slot_index, owner_display_name, owner_kind }` — the concrete payload of `Principal::UploadToken` (single source of truth; U2's `/u/{token}/*` extractor yields this).
- **Interface**: `authz_layer(State, Request, Next) -> Result<Response, ApiError>`; `AuthContext` and `UploadContext` as `FromRequestParts` extractors (401 if unauth, 403 if role insufficient); `require(ctx, allowed: &[Role]) -> Result<(), ApiError>`.

### C0.8 `shared::error::ApiError` — the single OkcError→HTTP mapping (Q9)
- **Responsibility**: the ONE module converting `EngineError` + okc-web-origin errors into HTTP responses, branching on `code`/`category` and NEVER on message strings. Emits a stable JSON body `{ code, category, message, retryable, retry_after_ms? }`. Synthesizes `Retry-After` for `PROJECT_BUSY`/`RESOURCE_LIMIT` by **code** (interop's `retryable` flag is inconsistent for reservation-collision `ProjectBusy`, so it is never trusted).
- **Interface**: `enum ApiError { Unauthenticated, Forbidden, NotFound, MethodNotAllowed, Validation(String), Engine(EngineError) }`; `From<EngineError> for ApiError`; `impl IntoResponse for ApiError`; `fn http_status(code: EngineErrorCode, category: EngineErrorCategory) -> StatusCode` (extended table incl. `ProjectInvalid→422`, `ArtifactSchemaUnsupported→422`, `PathUnsupported→400`, `OutputDurabilityUncertain→500`, safe default 500).

### C0.9 `shared::audit::AuditStore` + `CuratorDecision` — append-only decision record (Q8, C3 differentiator)
- **Responsibility**: append-only SQLite store recording each curator decision as an explicit typed event BEFORE it is mapped to an interop call, capturing principal, project, target, and **all three hash bindings** (`proposal_hash`, `critic_hash`, `taxonomy_hash`) for staleness/provenance (C-4), plus the resulting `JobId`. Records are never updated or deleted. **Single owner = U0** (`shared::audit`); records are *produced by* U4 and U3.
- **Interface**: `enum CuratorDecision { ApproveCluster{cluster_id, omission_rationales, minor_waivers}, RegenerateCluster{cluster_id, feedback}, ApproveTaxonomy{edited_clusters, rationale} }` — **deliberately NO winner-select variant** (ADR-0024, C-2; a verifiable C3 structural differentiator, confirmed by the verifier in both U0 and U4). `append(project, principal, decision, HashBindings) -> AuditRecord`; `list_for_project(project) -> Vec<AuditRecord>`. `struct HashBindings { proposal_hash: Option<String>, critic_hash: Option<String>, taxonomy_hash: Option<String> }`.

### `shared::state` — SQLite substrate (Q3)
- **C0.10 `StateDb`** — single-file SQLite pool (sqlx recommended for compile-time-checked queries; rusqlite acceptable), WAL mode, shared by every repository. Single-process topology → transactional integrity without external coordination.
- **C0.11 `Migrations`** — ordered, versioned DDL creating all tables + indices (see Application Design §"SQLite State Data Model").
- **C0.12 `SourceRegistry`** (repository over the `sources` table) — **resolves the previously dangling U5 dependency**. Maps okc-core `SourceId`/`DocumentId → owner_display_name` (+ `owner_kind`, `content_hash`, `absolute_path`, `slot_index`). **Owner = U3 orchestration** (project-registry domain); **written by U2** at `add_source` commit; **read by U5** `ProvenanceComposer` for owner-label enrichment. Interface: `record(SourceRow)`, `owner_labels(&[SourceId]) -> OwnerLabelMap`, `list_for_project(ProjectId)`.
...
- **`PasswordHasher`** (in `shared`, first consumed by U1): argon2id (bcrypt fallback), embedded per-hash salt + tuned params, constant-time verify (NFR-SEC-1). Plaintext lives only in `SecretString`.
...
- **`AuthProvider` impl (session)**: U1's concrete resolver injected into U0's `authz`. `resolve_session(raw_cookie_id) -> Result<Option<AuthContext>, AuthError>` performs the hashed lookup + role/curator-label bind, producing `Principal::Admin{...}`. (Authentication = U1; role enforcement = U0.)
...
- **`OrchestrationRoutes`** (axum handlers): E3 REST endpoints, cookie-session + admin-guarded. Includes the canonical **`GET /api/projects/{id}/jobs/{jobId}`** job-status poll (Q7).

---

## U4 — `review` (Epic E4)

- **`CuratorDecision`** (enum — the case-B model): 3 variants, NO winner-select variant (C3 differentiator; identical to C0.9). Every constructed decision is appended to U0's `AuditStore` (with `curator_id` + the hashes it was taken against) BEFORE the engine op; the receipt/`JobId` is linked after.
- **`DecisionGate`** (pure function): computes cluster review state from `critic.findings` in okc-web — counts Critical/Major (blocking, non-waivable) vs Minor (waivable) — and enforces the Major/Critical→regenerate-only rule BEFORE any `approve_cluster` engine call, returning a deterministic `422 APPROVAL_REQUIRED` when blocked. **Code-verified necessity**: core rejects blocking approvals with `AppError::InvalidProject("major or critical critic findings require regeneration")`, which interop's `map_project_message` maps to the generic `ProjectInvalid`, NOT `ApprovalRequired` — so the clean 422 must come from this okc-web gate. Interface: `evaluate(&CriticReportDto) -> GateVerdict`, `assert_approvable(&CriticReportDto) -> Result<(), EngineError>`.
- **`TaxonomyReviewService`**: loads the taxonomy proposal (E4-2), applies admin edits into `Vec<ClusterEdit>`, approves via `approve_taxonomy(edited, rationale)` (rationale required when edited).
- **`ClusterReviewService`**: the E4-3 focal engine. Loads `Vec<ClusterReviewView>` from `clusters()` (each = proposal + critic), builds the case-B decision surface, dispatches `ApproveCluster`/omission/Minor-waive through `DecisionGate` then `approve_cluster`. Builds `omission_rationales` keyed `"{document_id}:{target_id}"` (matches core `omission_key`) and `minor_waivers` keyed by Minor `finding_id`.
...
- **`ReviewRoutes`** (axum handlers): E4 REST endpoints, admin-only (contributor → 403).
...
- **`ServingDtos`**: `VaultTreeDto`, `VaultNodeDto`, `VaultNoteDto`, `ManifestSummaryDto`, `VerificationView`, `VerifyCheckRowDto`, `ProvenanceView`, `NoteProvenanceView`, `LineageGraphDto`(nodes+edges), `ContradictionViewDto`, `ContradictionClaimViewDto`, `ServingStatusDto`, `McpContractDto` — all serde with a schema-version-guarded field.
...
- **`queryKeys` + TanStack Query hooks** (`frontend/lib/api/queries.ts`): one hook per read surface. Server components render shell/static; client components own review/upload/polling (Q10).
