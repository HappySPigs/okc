...
**Depth**: comprehensive. Communication styles: in-process **trait calls** (U1–U6 → `OkcEngine`), **bounded mpsc + oneshot** (handlers → `EngineActor`), **HTTP REST + JSON** (frontend/mcp → axum), and **SQLite** (all repositories → one WAL file). No inter-process coordination (single long-lived engine process, Q6/Q9=A). **UI is frozen — this document defines wiring only, no UI redesign.**

---

## Dependency matrix (unit → depends on)

| From \ On | U0 adapter+queue | U0 shared (authz/error/jobs/audit/state) | U1 auth | U2 upload | U3 orch | U4 review | U5 serving | okc-interop |
|---|---|---|---|---|---|---|---|---|
| **U0 adapter** | — | jobs, audit (link JobId) | — | — | — | — | — | **links directly (sole point)** |
| **U0 shared** | error←EngineError | — | — | — | — | — | — | no |
| **U1 auth** | — | authz (supplies `AuthProvider` session resolver), error, state | — | — | curator label → U3 | — | — | no |
| **U2 upload** | engine-queue (`add_source`), adapter (`AddSourceRequest`→`SourceInput`) | authz (token resolver), error, jobs, audit, state (`sources` write) | needs admin session | — | reads project + sources root | — | — | no (via U0) |
...
- **U1 → U0 authz**: U1 provides the `AuthProvider::resolve_session` impl producing `Principal::Admin{account_id, session_id_hash, curator_label}`; U0 owns the canonical `AuthContext{principal, role}` + extractor + `require`. U0 authorizes, U1 authenticates.
...
- **All → U0 error**: every engine-touching op returns `EngineError`; the one `shared::error` module maps to HTTP.
...
1. **Trait boundary (in-process)** — U1–U6 depend only on `OkcEngine` + okc-web DTOs. Compile-time enforcement of ADR-0002: `okc_interop` is imported only in `adapter`.
2. **Serialization queue (mpsc + oneshot)** — reserving/mutating ops go handler → `EngineHandle` → `EngineActor` (single blocking owner) → interop. Non-reserving reads bypass the queue via a read-path `OkcClient` clone (verifier #8).
...
```mermaid
flowchart TD
    FE["U6 Next.js E1-E5 screens"] -->|"REST: cookie / bearer / read-only"| RT["U1-U5 axum handlers"]
    ...
    SVC -->|"non-reserving reads"| RC["read-path OkcClient clone, bypass queue"]
    Q --> ENG["U0 OkcEngine impl, schema v2 guard"]
    RC --> ENG
    ENG --> INT["okc-interop OkcClient / Project / Job"]
...
1. The U6 Next.js frontend calls U1–U5 axum handlers over REST using one of three auth contexts: admin cookie session, `/u/{token}` bearer token, or read-only `/api/serving/*`.
...
6. Reserving/mutating engine ops go through the U0 single-writer EngineQueue → the `OkcEngine` impl (schema-v2 guard) → okc-interop → okc-core, making okc-web the single writer. Non-reserving reads (taxonomy/clusters/manifest/verify/explain) bypass the queue via a read-path `OkcClient` clone so they never stall behind a long integrate/compile.
...
10. U5's CompiledVaultStore reads the immutable 3-root compiled vault (`knowledge/`+`legacy/`+`.okc/`) directly from disk; only the adapter reaches okc-interop/okc-core.
