...
Start by obtaining the current whole-file `sha256` from `read_note` (follow `nextOffset` to read a long note completely before editing it). Then choose the narrowest tool: `standardize_frontmatter` for title/aliases/tags, `update_note` for a body and/or arbitrary frontmatter change, `fix_yaml` to replace malformed frontmatter with a corrected block, and `reinforce_sources_links` to add literal source or link text. Every edit is applied in place — the first Unit performs no file move, rename, or merge. Preview with `dryRun: true`; applying with `false` first writes a single external pre-change backup and then returns a backup ID and the new hash. If a hash conflict occurs, re-read the latest content and review again. Do not edit the same note concurrently from an external editor. See [recovery.md](recovery.md) for restoring an earlier version from a backup.
