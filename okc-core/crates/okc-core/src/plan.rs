...
pub const INSPECTION_SCHEMA_VERSION: u32 = 2;
...
pub struct DraftPlan {
    ...
    pub resource_estimate: PlanResourceEstimate,
...
pub struct PlanResourceEstimate {
    pub source_files: u64,
    pub source_bytes: u64,
    pub note_count: u64,
    pub estimated_output_bytes: u64,
    pub estimated_peak_working_set_bytes: u64,
}
...
impl DraftPlan {
    ...
    pub fn validate_integrity(&self) -> Result<()> {
        ...
        if self.resource_estimate
            != calculate_resource_estimate(&self.snapshots, &self.workspace, &self.operations)?
        {
            return Err(OkcError::PlanStale(
                "plan resource estimate does not match its sealed inputs".into(),
            ));
        }
...
fn calculate_inspection_hash(
    ...
    canonical_hash("okc:inspection:v2\0", &identity)
}
...
struct PlanIdentity<'a> {
    ...
    resource_estimate: &'a PlanResourceEstimate,
...
fn validate_snapshot_set(snapshots: &[VaultSnapshot], policy: &CompilerPolicy) -> Result<()> {
    ...
    for snapshot in snapshots {
        ...
        let expected_vault_content_id = canonical_hash(
        ...
        if snapshot.vault_content_id != expected_vault_content_id
            || !vault_content_ids.insert(snapshot.vault_content_id)
        {
            return Err(OkcError::PlanStale(format!(
                "snapshot {} has a stale or duplicate Vault content identity",
                snapshot.snapshot_id
            )));
        }
        ...
        for file in &snapshot.files {
            ...
            total_files = total_files
                .checked_add(1)
                .ok_or_else(|| OkcError::ResourceLimit("plan file count overflow".into()))?;
            total_bytes = total_bytes
                .checked_add(file.byte_len)
                .ok_or_else(|| OkcError::ResourceLimit("plan byte count overflow".into()))?;
        }
    ...
    for (canvas_id, canvas) in &workspace.canvases {
        ...
        let parsed_references =
            crate::parse::canvas_file_references(&canvas.value, &canvas.source_file.logical_path)
                .map_err(|error| OkcError::PlanStale(error.to_string()))?;
    ...
    for (asset_id, asset) in &workspace.assets {
        ...
        for source in &asset.sources {
            ...
            let expected_asset_id =
                AssetId::from_parts("okc:asset:v2\0", &[source.content_hash.as_bytes()]);
...
pub fn build_plan(inspection: &Inspection, policy: &CompilerPolicy) -> Result<DraftPlan> {
    ...
    let resource_estimate =
        calculate_resource_estimate(&inspection.snapshots, &workspace, &operations)?;
...
fn calculate_resource_estimate(
    snapshots: &[VaultSnapshot],
    workspace: &CanonicalWorkspace,
    operations: &[OutputOperation],
) -> Result<PlanResourceEstimate> {
    let source_files = snapshots.iter().try_fold(0_u64, |total, snapshot| {
        total
            .checked_add(snapshot.files.len() as u64)
            .ok_or_else(|| OkcError::ResourceLimit("source file estimate overflow".into()))
    })?;
    let source_bytes = snapshots
        ...
        .try_fold(0_u64, |total, file| {
            total
                .checked_add(file.byte_len)
                .ok_or_else(|| OkcError::ResourceLimit("source byte estimate overflow".into()))
        })?;
    let mut estimated_output_bytes = 0_u64;
    let mut largest_file = 0_u64;
    for operation in operations {
        let (snapshot_id, source_path) = match operation {
            OutputOperation::Copy {
                snapshot_id,
                source_path,
                ..
        ...
        let byte_len = snapshots
            ...
            .ok_or_else(|| OkcError::PlanStale("operation estimate source is stale".into()))?
            .byte_len;
        estimated_output_bytes = estimated_output_bytes
            .checked_add(byte_len)
            .ok_or_else(|| OkcError::ResourceLimit("output byte estimate overflow".into()))?;
        largest_file = largest_file.max(byte_len);
    }
    let estimated_peak_working_set_bytes = largest_file
        .saturating_mul(2)
        .saturating_add(16 * 1024 * 1024);
    Ok(PlanResourceEstimate {
        source_files,
        source_bytes,
        note_count: workspace.documents.len() as u64,
...
fn allocate_asset_paths(
    ...
    for (asset_id, asset) in &workspace.assets {
        let source = asset.canonical_source().ok_or_else(|| {
            OkcError::Internal(format!("asset {asset_id} has no source occurrences"))
        })?;
    ...
    for (source_id, rewrites) in by_source {
        ...
        for rewrite in rewrites {
            ...
            let entry = source_entries
                ...
                .ok_or_else(|| {
                    OkcError::IdentityMismatch(format!(
...
fn render_rewritten_markdown(
    ...
    std::str::from_utf8(source).map_err(|error| OkcError::MalformedInput {
    ...
    for replacement in replacements {
        let start = usize::try_from(replacement.span.byte_start)
            .map_err(|_| OkcError::PlanStale("Markdown rewrite start overflow".into()))?;
        let end = usize::try_from(replacement.span.byte_end)
            .map_err(|_| OkcError::PlanStale("Markdown rewrite end overflow".into()))?;
        let link = document
            .links
            .iter()
            .find(|link| link.span == replacement.span)
            .ok_or_else(|| {
                OkcError::PlanStale("Markdown rewrite span is not bound to a sealed link".into())
            })?;
    ...
    std::str::from_utf8(&output).map_err(|error| {
        OkcError::PlanStale(format!("rewritten Markdown is not UTF-8: {error}"))
    })?;
...
fn validate_rewritten_markdown_links(
    ...
    output_file.byte_len = u64::try_from(output.len())
        .map_err(|_| OkcError::ResourceLimit("rewritten Markdown length overflow".into()))?;
...
pub(crate) fn render_rewritten_canvas(
    ...
    let nodes = rewritten_value
        ...
        .ok_or_else(|| {
            OkcError::PlanStale(format!("Canvas {} lost its node array", canvas.canvas_id))
        })?;
    for rewrite in rewrites {
        ...
        let target_is_sealed = match &reference.resolution {
            CanvasReferenceResolution::Resolved { target } => *target == rewrite.target,
            CanvasReferenceResolution::Ambiguous { candidates } => {
                candidates.contains(&rewrite.target)
            }
            CanvasReferenceResolution::Pending | CanvasReferenceResolution::Unresolved => false,
        };
        if reference.raw_path != rewrite.original_path || !target_is_sealed {
            return Err(OkcError::PlanStale(format!(
...
fn projection_hash(workspace: &CanonicalWorkspace) -> Result<ContentHash> {
    canonical_hash("okc:projection:v2\0", workspace)
}
