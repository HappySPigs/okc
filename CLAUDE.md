# OKC Repository AI-DLC Scope

This Git repository is the integration workspace for a multi-module project.
Apply these routing rules before AI-DLC workspace detection:

1. Every top-level `okc-*` directory is an independent AI-DLC project. It MUST keep its own `aidlc-docs/`, workflow state, audit history, and module-specific agent instructions inside that directory.
2. The repository-root `aidlc-docs/` is reserved for the umbrella project: module discovery, dependency and interface coordination, unified build and test orchestration, packaging, and release integration.
3. A request scoped to one module MUST use that module directory as the AI-DLC workspace root and write artifacts only to `<module>/aidlc-docs/`.
4. A request that changes repository-wide assembly or coordinates multiple modules MUST use this Git repository root as the AI-DLC workspace root and write integration artifacts to `/aidlc-docs/`.
